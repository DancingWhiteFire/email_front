...
interface UserAppState {
  user: UserType | null;
  authType: (typeof AUTH_VALUES)[keyof typeof AUTH_VALUES] | null;
  emails: Email[];
  tasks: Task[];
  ...
  authGoogle: (code: string) => Promise<void>;
  authVerify: (token?: string) => Promise<void>;
  logout: () => Promise<void>;
}
...
export const useStore = create<UserAppState>((set) => ({
  ...
  async authGoogle(credential: string) {
    set({ isLoading: true, authType: AUTH_VALUES.GOOGLE });
    try {
      ...
      apis.defaults.headers.common["Authorization"] = `Bearer ${result.token}`;
      localStorage.setItem("user", JSON.stringify(result.user));
    } catch (error) {
      console.error("Login failed:", error);
      alert("Login failed. Please try again.");
    } finally {
      set({ isLoading: false, authType: null });
    }
  },
